...
import (
	"github.com/AoC-Gamers/connect-libraries/auth-lib/config"
)
...
func NewAuthConfig(jwtSecret string) AuthConfig {
	return AuthConfig{
		JWTSecret:           jwtSecret,
		PolicyVersionGlobal: config.LoadPolicyVersion(),
	}
}
...
func NewAuthConfigWithFallback(jwtSecret string, fallbackVars ...string) AuthConfig {
	return AuthConfig{
		JWTSecret:           jwtSecret,
		PolicyVersionGlobal: config.LoadPolicyVersionWithFallback(fallbackVars...),
	}
}
